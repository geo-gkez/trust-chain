...
// Module-level flag — prevents listener accumulation across connect() calls
...
function loadContract(signer) {
  ...
  return markRaw(new Contract(CONTRACT_ADDRESS, TrustChainArtifact.abi, signer))
}

export const useWalletStore = defineStore('wallet', {
  state: () => ({
    account: null,       // string | null
    contract: null,      // Contract | null (markRaw)
    provider: null,      // BrowserProvider | null (markRaw)
  ...
  actions: {
    async connect() {
      ...
      try {
        ...
        // 3. Build signer + contract
        const browserProvider = markRaw(new BrowserProvider(window.ethereum, CHAIN_ID))
        const signer = await browserProvider.getSigner()
        this.provider = browserProvider
        this.contract = loadContract(signer)

        // 4. Register listeners once — prevents accumulation across account switches
